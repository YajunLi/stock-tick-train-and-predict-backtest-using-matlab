...
for i=1:length(uu)
    ...
    for k=0:data(end,1)
        if ~isempty(find(data(:,1)==k)) 
            xh=find(data(:,1)==k);  
            minute_high = [minute_high; max(data(xh,3))];
            minute_low = [minute_low; min(data(xh,3))];
            minute_close = [minute_close; data(xh(end),3)];
            minute_vol = [minute_vol; voll(xh(end))];
        else 
            minute_high = [minute_high; 0];
            minute_low = [minute_low; 0];
            minute_close = [minute_close; 0];
            minute_vol = [minute_vol; 0];
        end
    end  
     
           ATR_=ATR(minute_high,minute_low,minute_close,14,0);
           BIAS_=BIAS(minute_close,6,0);
           CR_=CR(minute_high,minute_low,minute_close,26);
           [Kvalue,Dvalue,Jvalue]=KDJ(minute_high,minute_low,minute_close,14,3,3,3);
           [DIF,DEA,MACDValue]=MACD(minute_close,12,26,9);
           MTM_=MTM(minute_close,12);
           OBV_=OBV(minute_close,minute_vol);
           PSY_=PSY(minute_close,12);
           RSI_=RSI(minute_close,14);
           VR_=VR(minute_close,minute_vol,26);
...
% unconditional factors
           ...
           factor(k+1,38)=ATR_(k+1);
           factor(k+1,39)=BIAS_(k+1);
           factor(k+1,40)=CR_(k+1);
           factor(k+1,41)=Kvalue(k+1);
           factor(k+1,42)=Dvalue(k+1);
           factor(k+1,43)=Jvalue(k+1);
           factor(k+1,44)=DIF(k+1);
           factor(k+1,45)=DEA(k+1);
           factor(k+1,46)=MACDValue(k+1);
           factor(k+1,47)=MTM_(k+1);
           factor(k+1,48)=OBV_(k+1);
           factor(k+1,49)=PSY_(k+1);
           factor(k+1,50)=RSI_(k+1);
           factor(1,51)=VR_(k+1);
